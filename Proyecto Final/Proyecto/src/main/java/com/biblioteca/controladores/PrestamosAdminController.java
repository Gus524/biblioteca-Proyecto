...
public class PrestamosAdminController {
    ...
    private void configurarFilas(){
        tbPrestamos.setRowFactory(t -> {
            TableRow<PrestamoUsuario> fila = new TableRow<>();
            fila.setOnMouseClicked(event -> {
                if(!fila.isEmpty()){
                    PrestamoUsuario p = fila.getItem();
                    try {
                        mostrarVerPrestamo(p);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
    ...
    @FXML
    private void mostrarVerPrestamo(PrestamoUsuario p) throws IOException{
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/biblioteca/vistas/modals/verPrestamo.fxml"));
        Parent root = loader.load();
        Stage stage = new Stage();
        stage.setTitle("Ver Prestamo");
        stage.initModality(Modality.APPLICATION_MODAL);
        stage.initOwner(null);
        stage.setScene(new Scene(root));
        AdministrarPrestamoController controller = loader.getController();
        controller.cargarPrestamo(p);
        stage.showAndWait();
    }
    ...
    private void actualizarTabla(){
        prestamos.clear();
        PrestamoUsuario p = new PrestamoUsuario();
        prestamos.addAll(p.obtenerPrestamos());
        tbPrestamos.refresh();
    }
}
