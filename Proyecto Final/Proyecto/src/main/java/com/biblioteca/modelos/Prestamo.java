...
public class Prestamo implements ConvertirMapeo{
    ...
    public Boolean comprobarPrestamoDevuelto(int id_estado){
        return (cnn.comprobar("SELECT * FROM Prestamo_Concentrado WHERE " +
                                "id_prestamo_user = ? AND id_estado = ?", 
                                getId_prestamo_user(), id_estado) > 0);
    }
}
